...
pub fn read16(c: *Cps, addr: u24) u16 {
    return switch (addr) {
        ...
        in1_lo...in1_hi => in1(c),
        // Byte-wide: the system inputs and, on a board that has them, three
        // banks of DIP switches this one does not.
        in0_lo...in0_hi => byteWide(if (addr & 6 == 0) in0(c) else 0xff) << 8 | 0xff,
...
pub fn write16(c: *Cps, addr: u24, value: u16) void {
    poke(c, addr, value, 0xffff);
}

pub fn write8(c: *Cps, addr: u24, value: u8) void {
    const even = addr & 1 == 0;
    const word = @as(u16, value) * 0x0101;
    poke(c, addr & ~@as(u24, 1), word, if (even) 0xff00 else 0x00ff);
}
...
fn pokeBytes(bytes: []u8, offset: u32, value: u16, mask: u16) void {
    ...
    if (mask & 0xff00 != 0) bytes[offset] = @truncate(value >> 8);
    if (mask & 0x00ff != 0) bytes[offset + 1] = @truncate(value);
}

fn pokeByteWide(bytes: []u8, offset: u32, value: u16, mask: u16) void {
    if (offset >= bytes.len or mask & 0x00ff == 0) return;
