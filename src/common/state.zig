...
/// file cannot carry.
pub fn Format(comptime M: type, comptime C: type) type {
    return struct {
        const machine_bytes = @sizeOf(M);
        const cpu_bytes = @sizeOf(C);

        /// A state file, exactly. Every one is the same size: the machine is a
        /// fixed struct and so is the file it goes in.
        pub const bytes = body_at + machine_bytes + cpu_bytes;

        /// What to read a state file with. One over the size and not the size
        /// itself, because `readFileAlloc` refuses a stream that *reaches* its
        /// limit — and every state reaches `bytes` on the nose, so the obvious
        /// cap rejects every file this module writes.
        ...
        pub fn save(c: *const M, cpu: *const C, out: *[bytes]u8) void {
...
/// A machine, as far as the format is concerned: the four fields whose contents
/// are the process's rather than the file's, and some bytes to prove moved.
const Fake = struct {
    board: board.Board,
    rom: romset.Set,
    sound: soundboard.SoundBoard = .{},
    mixer: audio.Mixer = .{},
    ram: [0x100]u8 = @splat(0),
    frame: u64 = 0,
};
const FakeCpu = struct { d: [8]u32 = @splat(0) };
const fake = Format(Fake, FakeCpu);
...
test "a state round-trips the machine and leaves the ROMs where they were" {
    const buf = try testing.allocator.create([fake.bytes]u8);
    defer testing.allocator.destroy(buf);
    const c = try testing.allocator.create(Fake);
    defer testing.allocator.destroy(c);
    const back = try testing.allocator.create(Fake);
    ...
    var back_cpu: FakeCpu = .{};
    try fake.load(back, &back_cpu, buf);

    try testing.expectEqual(@as(u8, 0xab), back.ram[0x34]);
...
test "a state that is not one, is from another build, or is short is refused" {
    ...
    var cpu: FakeCpu = .{};
    fake.save(c, &cpu, buf);
    try fake.load(c, &cpu, buf);

    try testing.expectError(error.NotASaveState, fake.load(c, &cpu, "not a state at all"));
    try testing.expectError(error.Truncated, fake.load(c, &cpu, buf[0 .. fake.bytes - 1]));

    var wrong = buf.*;
    wrong[layout_at] ^= 1;
    try testing.expectError(error.FromAnotherBuild, fake.load(c, &cpu, &wrong));
    wrong = buf.*;
    wrong[version_at] +%= 1;
    try testing.expectError(error.WrongVersion, fake.load(c, &cpu, &wrong));
}
...
test "the layout hash follows the machine's shape rather than its contents" {
    ...
    const machine = comptime layout(Fake);
